...
connect_string = "sqlite:///data/climateDB.db"
# connect_string = "sqlite:///C:\\Users\\user\\Downloads\\Climate-Interactive-Dashboard-main\\Climate-Interactive-Dashboard-main\\static\\data\\climateDB.db"
...
demo_sorted_df = demo_sorted_df.rename(
    {
        "name": "Area",
...
def launchPage():   
    ...
    popu_data = popu_data[
        ['Area', 'Population', 'Density', 'Land Size', 'Images', 'Lat', 'Lng']
        ].fillna(0).set_index("Area", drop=True)
...
def get_season(country='United States of America'):
    ...
    season_country_mean = season_country_mean.drop(['field1', 'Element', 'Unit'], axis=1).reset_index(drop=True)
...
def get_months(country='United States of America'):
    ...
    months_country_mean = months_country_mean.drop(['field1', 'Area', 'Element', 'Unit'], axis=1)
    # Get years data
...
def get_scatter(country='United States of America'):
    ...
    temp_country = meteor_df.loc[meteor_df['Area'] == country].drop(['field1', 'Area','Months', 'Element', 'Unit'], axis=1).reset_index(drop=True).T.reset_index()
    ...
    temp_country['Year'] = pd.to_datetime(temp_country['Year'].map(str),format="%Y")
    temp_country['Year'] = pd.DatetimeIndex(temp_country['Year']).year

    # get Co2 Emission Data , drop unwated fields and reset index
    co2_country = emission_df.loc[emission_df['Entity']==country].drop(['Entity', 'Code'], axis=1).reset_index(drop=True)
    ...
    Scatter_obj = {
        ...
        "Avg Temp Change": list(np.ravel(merged_Temp_co2.get('Avg Temp', []))),
